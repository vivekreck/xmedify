describe("Hospital", () => {
  beforeEach(() => {
    cy.visit("/"); // Adjust the URL to match your application
  });
  ...
  it("should display all booked slots on the My Bookings page", () => {
    ...
    cy.visit("/my-bookings"); // Replace with the actual URL for the My Bookings page
  ...
  it("should persist bookings in localStorage after page reload", () => {
    ...
    cy.visit("/my-bookings"); // Replace with actual My Bookings URL

    cy.get("h3").contains("southeast alabama medical center", { timeout: 5000 }).should("be.visible");
